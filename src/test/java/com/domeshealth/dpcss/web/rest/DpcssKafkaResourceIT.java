package com.domeshealth.dpcss.web.rest;
...
import com.domeshealth.dpcss.config.KafkaProperties;
...
class DpcssKafkaResourceIT {
    ...
    void setup() {
        ...
        DpcssKafkaResource kafkaResource = new DpcssKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/dpcss-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/dpcss-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
